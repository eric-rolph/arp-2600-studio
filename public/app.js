...
function renderTakes(){
  $('#record').classList.toggle('active',tape.recording);$('#record').textContent=tape.recording?'● Recording':'● Record';$('#tape-play').classList.toggle('active',tape.playing);$('#tape-lamp').classList.toggle('recording',tape.recording);$('#tape-status').textContent=tape.recording?'RECORDING':tape.playing?'PLAYING':'STOPPED';$('#take-count').textContent=`${tape.takes.length} TAKE${tape.takes.length===1?'':'S'}`;
